...
const int azul = 4;
const long HoraAmanecer = 10, MinutoAmanecer = 50, SegundoAmanecer = 10;
const long HoraManana = 10, MinutoManana = 50, SegundoManana = 20;
const long HoraMediodia = 10, MinutoMediodia = 50, SegundoMediodia = 30;
const long HoraTarde = 10, MinutoTarde = 50, SegundoTarde = 40;
const long HoraAtardecer = 10, MinutoAtardecer = 50, SegundoAtardecer = 50;
const long HoraNoche = 10, MinutoNoche = 51, SegundoNoche = 0;
...
boolean esAmanecer(time_t amanecer){
  long hora_actual = hour(amanecer)*3600 + minute(amanecer)*60 + second(amanecer);
...
boolean esManana(time_t manana){
  long hora_actual = hour(manana)*3600 + minute(manana)*60 + second(manana);
...
boolean esMediodia(time_t mediodia){
  long hora_actual = hour(mediodia)*3600 + minute(mediodia)*60 + second(mediodia);
  long hora_mediodia = HoraMediodia*3600+MinutoMediodia*60+SegundoMediodia;
...
boolean esTarde(time_t tarde){
  long hora_actual = hour(tarde)*3600 + minute(tarde)*60 + second(tarde);
...
boolean esAtardecer(time_t atardecer){
  long hora_actual = hour(atardecer)*3600 + minute(atardecer)*60 + second(atardecer);
  long hora_atardecer = HoraAtardecer*3600+MinutoAtardecer*60+SegundoAtardecer;
...
boolean esNoche(time_t noche){
  long hora_actual = hour(noche)*3600 + minute(noche)*60 + second(noche);
  long hora_amanecer = HoraAmanecer*3600+MinutoAmanecer*60+SegundoAmanecer;
  long hora_noche = HoraNoche*3600+MinutoNoche*60+SegundoNoche;
